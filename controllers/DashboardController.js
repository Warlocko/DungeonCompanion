  ...
  EventModel.findById(evid)
    .then((event) => {
      ResponseModel.findByEventId(evid)
        .then((responses) => {
          res.render('dashboard/event', { cmpid: cmpid, event: event, responses: responses, user: req.user });
        }).catch((responses) => {
          res.render('dashboard/event', { cmpid: cmpid, event: event, responses: responses, user: req.user });
        })
    }).catch((event) => {
...
exports.newresult = (req,res) => {
  res.render('dashboard/result-form', { cmpid: req.params.cmpid, evid: req.params.evid, responseid: req.params.responseid })
}

exports.resultsend = (req, res) => {
  cmpid = req.params.cmpid
  evid = req.params.evid
  response_id = req.params.responseid
  ResponseModel.addResult(req.body.description, response_id).then(() => {
    res.redirect(`/app/campaign/${cmpid}/event/${evid}`)
  })
}
